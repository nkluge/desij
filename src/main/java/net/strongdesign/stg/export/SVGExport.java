...
public class SVGExport {
    ...
    private static LinkedList<Element> createTransition(Document doc, STG stg, Transition t) {
        ...
        switch(stg.getSignature(t.getLabel().getSignal())) {
            ...
            case INPUT:
                ...
                underline.setAttribute("d", "M " + (p.x - 20) + "," + (p.y + 8) + " l 40,0");
                break;
            case OUTPUT:
                style = outputStyle;
                break;
    ...
    private static Element createConnection(Document doc, STG stg, net.strongdesign.stg.Node n1, net.strongdesign.stg.Node n2, int w) {
        ...
        line.setAttribute("d", "M " + (double)Math.round(x1 * 10) / 10 + "," + (double)Math.round(y1 * 10) / 10 + " L " + (double)Math.round(x2 * 10) / 10 + "," + (double)Math.round(y2 * 10) / 10);

        line.setAttribute("marker-end", "url(#PNArrow)");
        return line;
    }

    private static LinkedList<Element> createConnections(Document doc, STG stg, Place place) {
        LinkedList<Element> result = new LinkedList<Element>();

        if(STGGraphComponent.isShorthandPlace(place, null)) {
            net.strongdesign.stg.Node n1 = place.getParents().iterator().next();
            net.strongdesign.stg.Node n2 = place.getChildren().iterator().next();
            result.add(createConnection(doc, stg, n1, n2, 1));

        } else {

            for(net.strongdesign.stg.Node n : place.getParents())
                result.add(createConnection(doc, stg, n, place, n.getChildValue(place)));

            for(net.strongdesign.stg.Node n : place.getChildren())
                result.add(createConnection(doc, stg, place, n, place.getChildValue(n)));
    ...
    public static String export(STG stg) {
        ...
        try {
            ...
            desc.appendChild(doc.createTextNode(Messages.getString("STGFile.stg_start_comment") + " " + new Date()));
